...
def run_analysis(cleaned_text: str):
    for attempt in range(1, 4):
        try:
            ...
            st.session_state.ats = ats_chain.invoke({})
            time.sleep(1)
            skill_chain = extract_skills_from_resume_chain(cleaned_text, llm)
            st.session_state.skills = skill_chain.invoke({})
            return
        except Exception as e:
            err_msg = str(e)
            if any(term in err_msg for term in ("503", "429", "UNAVAILABLE", "RESOURCE_EXHAUSTED", "connection", "forcibly closed")):
                if attempt < 3:
                    wait_sec = attempt * 5
                    st.warning(f"⏳ Google Gemini servers are busy (503/429). Retrying attempt {attempt+1}/3 in {wait_sec}s...")
                    time.sleep(wait_sec)
                else:
                    st.error(f"Analysis encountered an API issue: {err_msg}")
            else:
                raise e
